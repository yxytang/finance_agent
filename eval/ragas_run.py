...
def write_findings(rows, names, samples, elapsed) -> None:
    ...
    body += [
        ...
        "2. **检索是混合的**（两路 BM25 + 一路向量 + 重排），而**哪几路生效取决于"
        "跑的时候配没配 `RAG_API_KEY`**。没配就退回纯词法，成绩会不一样 ——"
        "所以看这份报告时要先确认那次是怎么跑的。"
        "纯词法那一路的短板在 Day 5 用原始问题直接查检索器时量到过（转述组 1/5）；"
        "第十一天补上向量 + 重排之后同一个面变成 3/5、平均排名 2.20。"
        "**至于这份报告里转述组的失分该算在检索头上还是路由头上，看上面那节的"
        "表** —— 那张表是按本次数据算出来的。这里不写死一句结论："
        "第十一天就写死过一次（「4 道里 3 道没调检索」），下一次跑路由变好了，"
        "那句话当场变成假的，而它看起来仍然像一份分析。",
